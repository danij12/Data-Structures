...
from pickle import NONE, TRUE, FALSE
...
class List:
    ...
    def combine(self, source1, source2):
        ...
        """
        for a in source1:
            self._values.append(deepcopy(a))
        for b in source2:
            self._values.append(deepcopy(b))
    ...
    def intersection(self, source1, source2):
        ...
        if len(source1._values) > 0 and len(source2._values) > 0:
            for a in source1._values:
                if a in source2._values:
                    temp.append(a)
        for c in temp:
            if c not in temp2:
                temp2.append(c)
    ...
    def split(self):
        ...
        if not self.is_empty():
            half_length = len(self._values)//2
            for a in range(half_length):
                 target1.append(self.remove_front())
            for b in range(len(self._values)):
    ...
    def split_alt(self):
        ...
        while not self.is_empty():
            target1.append(deepcopy(self.remove_front()))
            target2.append(deepcopy(self.remove_front()))
        return target1, target2
